...
mod tests {
    ...
    use crate::BatchFunctions::BatchSort;
    use rayon::prelude::*;
    ...
    fn test_medium_array_to_InMemory() {
...

        {
            ...
            for _i in 0..chunk_count {
                let (arr, vec) = create_fingerprints_vector(width, height);
                test_vec.extend(vec);
                table.add_chunk(&arr);
            }
        }
        ...
        println!("The following print is the sorted array from the batchsorter");
        {
            ...
            for i in 0..chunk_count {
                let _arr: Array2<i32> = table.load_chunk(i);
                //array_helper::print_array_2D(&arr);
            }
        }
    ...
    fn test_medium_array_to_InMemory_differentSizes() {
...

        {
            ...
            let (arr, vec) = create_fingerprints_vector(width, 8);
            test_vec.extend(vec);
            table.add_chunk(&arr);
    ...
    fn test_medium_array_to_DirectIO() {
        ...
        println!("The following print is the sorted array from the batchsorter");
        for i in 0..chunk_count {
            let _arr: Array2<i32> = table.read().unwrap().load_chunk(i);
            //array_helper::print_array_2D(&arr);
        }
