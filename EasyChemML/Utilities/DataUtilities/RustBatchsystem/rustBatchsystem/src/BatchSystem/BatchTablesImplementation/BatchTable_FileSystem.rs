...
impl BatchTable_FileSystem {
    fn write_file<T: Data, D: Dimension + Serialize>(
        &mut self,
        array: &ArrayBase<T, D>,
        path: PathBuf,
    ) where
        <T as RawData>::Elem: serde::ser::Serialize,
    {
        ...
        let output_file = File::create(path.clone()).expect(&*format!(
    ...
    fn read_file<'de, T: DeserializeOwned, D: Dimension + DeserializeOwned>(
        &self,
        path: PathBuf,
    ) -> Array<T, D> {
        let mut decoder =
            Decoder::new(File::open(path.clone()).unwrap_or_else(|_| {
                panic!("Cannot find Path: {}", path.clone().to_str().unwrap())
            }))
            .unwrap();
        let mut file = Cursor::new(Vec::new());
        io::copy(&mut decoder, &mut file).unwrap();

        Serializer::from_vec(file, SerializerAlgo::SerdeJson)
    }
}
