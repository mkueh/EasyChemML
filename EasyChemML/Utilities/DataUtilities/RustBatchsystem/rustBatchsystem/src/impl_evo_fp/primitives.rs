...
impl Primitives {
    pub fn new(mol_data: &Vec<ROMol>, bool_matching: bool) -> Primitives {
        println!("Staring with primtives");
        ...
        for chirality in ["@", "@@"] {
            if is_primitive_usefull(chirality, mol_data, bool_matching) {
    ...
    pub fn generate_primitive_pattern(
        ...
        loop {
            let mut pattern_stri = String::from("[");
            for i in 0..primitive_count {
                ...
                while pattern_stri.contains(primitive) {
                    primitive = primitive_iter.clone().choose(&mut rng).unwrap();
                }

                random_percentage = rng.gen();
                if random_percentage > 0.5 {
                    pattern_stri.push('!');
                }
                pattern_stri.push_str(primitive);
                ...
                if i != primitive_count - 1 {
                    pattern_stri.push(*logic_bi_operator)
                }
            }
            pattern_stri.push(']');

            if is_smarts_valid(&pattern_stri) {
                return (pattern_stri, step_counter);
            }
            step_counter += 1;
        }
    }
