...
impl Population {
    ...
    pub fn save_population(&self, path: &str, step: i8) -> Result<(), std::io::Error> {
    ...
    pub fn get_saved_population(path: &str, found_max_step: i8) -> Result<Population, std::io::Error> {
        let file_path = &format!("{}/evolution_step_{}.bin", path, found_max_step);
        let reader = BufReader::new(File::open(file_path)?);
        let deserialized_population: Population = bincode::deserialize_from(reader)?;
        Ok(deserialized_population)
    }

    pub fn get_highest_saved_population(path: impl AsRef<Path>) -> Result<Population, std::io::Error> {
        let newest_file = DirEntry::new();
        let mut found_max_step = 0;

        for entry in WalkDir::new(path).into_iter().filter_map(|e| e.ok()) {
            if entry.file_type().is_file() && entry.path().extension()? == "bin" {
                let step = entry.file_name().chars().last()?;
        ...
        if newest_file.path().is_empty() {
Err()
        }
        let deserialized_population = Population::get_saved_population(newest_file.path(), found_max_step)?;
        Ok(deserialized_population)
    }
}
...
mod tests {
    ...
    fn test_save_population() {
        ...
        let population = generate_new_population(10, 5, 5, 5, false, &features);
        let temp_dir = tempdir().unwrap();
        let path = temp_dir.path().to_str().unwrap();
        let step = 3;
        let saving_result = population.save_population(path, step);
        let file_path = &format!("{}/evolution_step_{}.bin", path, step);
        let file = File::open(file_path).unwrap();
        let deserialized_population: Population = bincode::deserialize_from(file).unwrap();
        assert_eq!(population.members.len(), deserialized_population.members.len());
        assert_eq!(population.members[0].fingerprint.patterns, deserialized_population.members[0].fingerprint.patterns);
