...
impl Metric {
    pub fn get_direction(&self) -> MetricDirection {
        match self {
            Metric::ClassificationMetric(classification_metric) => {
                classification_metric.get_direction()
            }
            Metric::RegressionMetric(regression_metric) => regression_metric.get_direction(),
        }
    }
}
#[derive(Clone, Debug)]
...
impl ClassificationMetric {
    fn calculate_metric(
        &self,
        prediction_data: &Vec<(Array1<usize>, Option<Array1<f32>>)>,
        test_folds: &Vec<Dataset<usize, usize, Ix1>>,
    ) -> Vec<f32> {
        match self {
    ...
    fn get_direction(&self) -> MetricDirection {
        match self {
...
impl RegressionMetric {
    fn calculate_metric(
        &self,
        prediction_data: &Vec<(Array1<f64>, Option<Array1<f32>>)>,
        test_folds: &Vec<Dataset<f64, f64, Ix1>>,
    ) -> Vec<f32> {
        match self {
...
fn calculate_max_error(
    predictions: &Vec<(Array1<f64>, Option<Array1<f32>>)>,
    test_folds: &Vec<Dataset<f64, f64, Ix1>>,
) -> Vec<f32> {
