...
class XLSX(Abstract_DataImporter):
    # params
    _path: str
    _columns: List[str]
    ...
    def __init__(self, path: str, sheet_name: str, columns: List[str] = None, range: Tuple[int, int] = None):
        super().__init__()
        self._path = path
    ...
    def _load(self):
        if self._range is None:
            panda_dataframe = pandas.read_excel(self._path, usecols=self._columns, index_col=False, sheet_name=self._sheet_name)
        else:
