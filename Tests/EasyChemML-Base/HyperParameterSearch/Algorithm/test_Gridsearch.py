...
from EasyChemML.HyperparameterSearch.Algorithm.GridSearch import GridSearch
...
def test_gridsearch():
    ...
    load_dataset = {}
    load_dataset['doyle_Test1'] = XLSX('../../../_TestDataset/DreherDoyle.xlsx',
                                       sheet_name='Test1',
    ...
    splitter_doyle_Test1_inner = RangeSplitter(10, 50)
    ...
    catboost_r_doyle = Config(CatBoost_r, {'verbose': False, 'thread_count': thread_count, 'allow_writing_files': False,
                                           'iterations': IntRange(1, 5, 1)})
    randomForest_r_doyle = Config(scikit_RandomForestRegressor, {'n_estimators': IntRange(1, 5, 1)})
    Configs = ConfigStack([catboost_r_doyle,randomForest_r_doyle])

    hyperParameterSearch = GridSearch(env, 'Gridsearch_Results.csv')
    best_config = hyperParameterSearch.performe_HyperparamterSearch(Configs, job_runner, dataset_doyle_Test1, r2score)
