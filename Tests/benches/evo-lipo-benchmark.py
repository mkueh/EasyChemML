...
thread_count = 1
load_dataset = {'lipo': XLSX('../../Examples/_DATASETS/Lipophilicity.xlsx', sheet_name='Sheet1',
...
for i in range(10):
    ...
    MolRdkitConverter().convert(datatable=dh['lipo'], columns=['Smiles'], n_jobs=12)
    ...
    splitter_lipo = RangeSplitter(1, 1200)
    ...
    modelwrapper: ModelWrapper_Fitness = ModelWrapper_Fitness(train_dataset=dataset_lipo.to_SharedDataset(),
                                                              feature_typ=FeatureTyp.count_feature, metric=R2_Score(),
                                                              model=catboost_r_lipo)
